...
{
    printf("Usage: %s [--duration-sec N] [--save-every-sec N] [--no-save] [--save-first-only]\n", argv0);
...
static void *image_save_worker_main(void *arg)
{
    ...
    for (;;) {
        ...
        snprintf(path, sizeof(path), "worker_long_run_fe%d_%u_%lu.pgm",
                 req.sensor_id == T265_SENSOR_ID_FISHEYE0 ? 0 : 1,
                 req.frame_id,
                 (unsigned long)req.save_index);
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; ++i) {
        if (strcmp(argv[i], "--duration-sec") == 0) {
            ...
            save_enabled = 1;
        } else if (strcmp(argv[i], "--save-first-only") == 0) {
            save_every_sec = 0;
            save_enabled = 1;
    ...
    printf("--- T265 Image Save Worker Probe ---\n");
    ...
    printf("\nImage save worker probe summary:\n");
    printf("  duration_sec:             %d\n", duration_sec);
    ...
    printf("\nResult:\n");
    printf("  IMAGE_SAVE_WORKER_PROBE_RESULT: %s\n", result);
